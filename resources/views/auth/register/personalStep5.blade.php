...
.panel-tag-primary {
	border-left: 3px solid #0C83E2;
}
.panel-tag-danger {
	border-left: 3px solid #FD1381;
}
</style>
@endsection
@section('content')
								...
								<section class="pt-2 pl-4 pr-4">
									...
									<div class="panel-tag panel-tag-danger mt-2 bg-white">
										...
										<div class="form-row">
											...
											<div class="col-md-8 mb-3">
												<label class="form-label">ที่อยู่ <span class="text-danger">*</span> </label>
												<input type="text" class="form-control form-control-sm border-dashed" value="{{ $userData->contact_address ?? "" }} {{ " ตำบล ".$sub_districts[$userData->contact_sub_district] ?? "" }} {{ " อำเภอ ".$districts[$userData->contact_district] ?? "" }} {{ " จังหวัด ".$provinces[$userData->contact_province] ?? "" }} {{ $userData->contact_postcode ?? "" }}" readonly>
											</div>
										</div>
									</div>
								</section>
