			...
			<table>
				...
				<tr>
					<td colspan="2">
						...
						<div class="w-100">
							<div class="d-inline-block w-50 m-0 p-0">1. ชื่อผู้รับ {{ $data['report_result_receive_first_name']." ".$data['report_result_receive_last_name'] }}</div>
							<div class="d-inline-block w-30 m-0 p-0">โทร. {{ $data['report_result_receive_mobile'] }}</div>
						</div>
						<div class="w-100 mt-n10">
							<div class="d-inline-block">2. ที่อยู่: {{ $data['report_result_receive_addr'] }}</div>
						</div>
					</td>
				</tr>
				<tr>
					...
					<td colspan="2">
						...
						<div class="w-100 pt-4">
							...
							<div class="d-inline-block mr-40 mt-8">
								<div class="chk-box"><span class="chk-mark">{{ ($data['sample_sumary']['sample_completed'] > 0) ? '✔' : '' }} </span></div><span style="display: inline-block; margin-right: 22px; width: 120px;">&nbsp;สมบูรณ์:</span>
								<span>จำนวน {{ $data['sample_sumary']['sample_completed'] }} ตัวอย่าง {{ $data['sample_sumary']['sample_completed_amount'] }} พารามิเตอร์</span>
							</div>
							<div style="padding: 0 0 4px 100px;">
								<div class="chk-box"><span class="chk-mark">{{ ($data['sample_sumary']['sample_not_completed'] > 0) ? '✔' : '' }} </span></div><span style="display: inline-block; margin-right: 22px; width: 120px;">&nbsp;ไม่สมบูรณ์:</span>
								<span>จำนวน {{ $data['sample_sumary']['sample_not_completed'] }} ตัวอย่าง {{ $data['sample_sumary']['sample_not_completed_amount'] }} พารามิเตอร์ (ปฎิเสธการรับตัวอย่าง)</span>
							</div>
						</div>
						<div class="w-100">เนื่องจาก: {{ $data['sample_verify_desc'] }}</div>
						<div class="w-100">
							<div class="d-inline-block w-30">ผู้รับตัวอย่าง: {{ $data['received_order_name'] }}</div>
							<div class="d-inline-block w-30">วันที่: {{ $data['received_order_date'] }}</div>
						</div>
						<div class="w-100 mt-n4">
							<div class="d-inline-block w-30">ผู้ทบทวนคำขอ: {{ $data['review_order_name'] }}</div>
							<div class="d-inline-block w-30">วันที่: {{ $data['review_order_date'] }}</div>
						</div>
					</td>
				</tr>
				<tr>
